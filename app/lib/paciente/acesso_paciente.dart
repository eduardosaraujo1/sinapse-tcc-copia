...
// --- CONFIGURAÇÃO DA API ---
const String _baseUrl = 'http://localhost:8000/api/paciente/cadastrocompleto'; // Base para as rotas do paciente
...
class _RegistraPacienteScreenState extends State<RegistraPacienteScreen> {
  ...
  final TextEditingController _emailController = TextEditingController(); // NOVO
  final TextEditingController _passwordController = TextEditingController(); // NOVO
  ...
  Future<void> _submitRegistration() async {
    ...
    final Map<String, dynamic> data = {
      ...
      'peso': double.tryParse(_weightController.text.trim().replaceAll(',', '.')),
  ...
  Widget _buildTextField(String hint, TextEditingController controller, String? Function(String?) validator,
    ...
    return TextFormField(
      ...
      obscureText: isPassword && !_isPasswordVisible, // Usado apenas para o campo de senha
      keyboardType: type,
      validator: validator,
      decoration: InputDecoration(
        ...
        suffixIcon: isPassword 
            ? IconButton( // Botão de visibilidade de senha
                icon: Icon(
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              _buildTextField("Nome completo do paciente", _nameController, 
              ...
              const SizedBox(height: 8),
              _buildTextField("email@exemplo.com", _emailController, 
                (value) {
                  if (value!.isEmpty) {
              ...
              const SizedBox(height: 8),
              _buildTextField("Digite sua senha", _passwordController, 
                (value) {
                  if (value!.isEmpty) {
              ...
              // Idade
              const Text("Idade (anos)", style: TextStyle(fontSize: 16, color: Colors.black54)),
              const SizedBox(height: 8),
              _buildTextField("Digite a idade", _ageController, 
                (value) {
              ...
              // Peso
              const Text("Peso (Kg)", style: TextStyle(fontSize: 16, color: Colors.black54)),
              const SizedBox(height: 8),
              ...
              const Text("Tipo Sanguíneo *", style: TextStyle(fontSize: 16, color: Colors.black54)),
              const SizedBox(height: 8),
              DropdownButtonFormField<String>(
                ...
                validator: (value) => value == null ? 'O tipo sanguíneo é obrigatório.' : null,
              ),
              const SizedBox(height: 20),

              // Comorbidade
              const Text("Comorbidade (Ex: Diabetes, Hipertensão)", style: TextStyle(fontSize: 16, color: Colors.black54)),
              const SizedBox(height: 8),
              _buildTextField("Informe comorbidades ou deixe vazio", _comorbidadeController, 
