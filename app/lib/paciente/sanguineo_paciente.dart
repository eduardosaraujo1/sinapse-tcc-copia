...
import 'package:http/http.dart' as http; // Importação necessária para requisições HTTP
...
// --- CONFIGURAÇÃO DA API ---
const String patientBloodTypeApiUrl = 'http://localhost:8000/api/paciente/sanguineo';
...
class _SanguineoPacienteState extends State<SanguineoPaciente> {
 ...
 String? _selectedRhFactor;
  ...
  Future<void> sendBloodTypeToApi(String bloodType) async {
    ...
    print('Enviando tipo sanguíneo: $bloodType para a API: $patientBloodTypeApiUrl');
 ...
 Widget _buildRhFactorButton(String factor) {
  ...
  bool get _isSelectionComplete => _selectedBloodType != null && _selectedRhFactor != null;
      ...
      children: [
       ...
       SizedBox(
        ...
        child: ElevatedButton(
         ...
         onPressed: (_isSelectionComplete && !_isLoading) ? () async {
          setState(() {
           _isLoading = true; // Inicia o carregamento
          });

                    // Concatena o tipo e o fator Rh (ex: "A" + "+")
                    final String bloodType = _selectedBloodType! + _selectedRhFactor!;
          ...
          try {
           ...
           if (mounted) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
             const SnackBar(content: Text('Falha ao salvar o tipo sanguíneo. Verifique a conexão com a API.')),
            );
           }
          } finally {
           if(mounted) {
