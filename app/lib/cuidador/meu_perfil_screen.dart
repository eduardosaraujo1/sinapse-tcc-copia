...
class _MeuPerfilScreenState extends State<MeuPerfilScreen> {
  ...
  late Future<CuidadorPerfil> _perfilFuture;

  // URL da API. Use o IP especial 10.0.2.2 para emuladores Android
  // ou 'localhost' se estiver testando no Chrome/Web.
  ...
  Future<CuidadorPerfil> _fetchProfileData() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CuidadorPerfil.fromJson(data);
      } else {
  ...
  String _getInitialLetter(CuidadorPerfil perfil) {
    if (perfil.nome.isEmpty || perfil.nome == 'Nome Desconhecido') {
      return 'U';
    }
    return perfil.nome[0].toUpperCase();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<CuidadorPerfil>(
            ...
            return Padding(
              ...
              child: Column(
                children: [
                  ...
                  CircleAvatar(
                    radius: 50,
                    backgroundColor: Color.fromARGB(255, 106, 186, 213),
                    child: Text(
                      _getInitialLetter(perfil),
                      style: TextStyle(
                        fontSize: 36,
