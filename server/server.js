...
`;
    ...
    comorbidade
            ...
            if (error.code === 'ER_DUP_ENTRY') {
                return res.status(409).json({ error: 'Um registro com o campo único (ex: CPF, ID, etc.) fornecido já existe.' });
            }
...
app.post('/api/medicamentos', async (req, res) => {
    try {
        ...
        const [patientResults] = await pool.execute(
            'SELECT id, cuidador_id FROM pacientes WHERE nome = ?', 
            [patient_name]
        );
...
app.put('/api/cuidador/alterar-senha', async (req, res) => {
  ...
  try {
    ...
    const [cuidadores] = await connection.execute(
      'SELECT id, senha FROM cuidador WHERE email = ?',
      [email]
    );
    ...
    await connection.execute(
      'UPDATE cuidador SET senha = ? WHERE email = ?',
      [novaSenha, email]
    );
...
app.put('/api/pacientes/alterar-senha', async (req, res) => {
  ...
  try {
    ...
    const [pacientes] = await connection.execute(
      'SELECT id, senha FROM pacientes WHERE email = ?',
      [email]
    );
    ...
    if (senhaAtual !== paciente.senha) {  // ← Era "pacientes.senha" (array)
      return res.status(401).json({ 
        success: false, 
        message: 'Senha atual incorreta' 
      });
    ...
    // Atualizar a senha
    await connection.execute(
      'UPDATE pacientes SET senha = ? WHERE email = ?',
      [novaSenha, email]
    );
...
app.put('/api/familiares/alterar-senha', async (req, res) => {
  ...
  try {
    ...
    const [familiares] = await connection.execute(
      'SELECT id, senha FROM familiares WHERE email = ?',
      [email]
    );
    ...
    // Atualizar a senha
    await connection.execute(
      'UPDATE familiares SET senha = ? WHERE email = ?',
      [novaSenha, email]
    );
...
app.post('/api/delete-account', async (req, res) => {
    try {
        ...
        const [user] = await pool.execute(
            'SELECT id, email FROM cuidador WHERE id = ?',
            [userId]
        );
        ...
        try {
            ...
            await connection.execute(
                'DELETE FROM tarefas WHERE cuidador_id = ?',
                [userId]
            );

            // 3. Deletar consultas dos pacientes do cuidador
            await connection.execute(
                'DELETE FROM consultas WHERE cuidador_id = ?',
                [userId]
            );

            // 4. Deletar agendamentos_medicamentos dos pacientes do cuidador
            await connection.execute(
                'DELETE FROM agendamentos_medicamentos WHERE cuidador_id = ?',
                [userId]
            );

            // 5. Deletar pacientes do cuidador
            await connection.execute(
                'DELETE FROM pacientes WHERE cuidador_id = ?',
                [userId]
            );
...
app.post('/api/familiar/delete-account', async (req, res) => {
    try {
        ...
        const [user] = await pool.execute(
            'SELECT id, email FROM familiares WHERE id = ?',
            [familiarId]
        );
...
app.post('/api/paciente/delete-account', async (req, res) => {
    try {
        ...
        const [user] = await pool.execute(
            'SELECT id, email, nome FROM pacientes WHERE id = ?',
            [pacienteId]
        );
        ...
        try {
            ...
            await connection.execute(
                'DELETE FROM registros_diarios WHERE paciente_id = ?',
                [pacienteId]
            );
            ...
            await connection.execute(
                'DELETE FROM agendamentos_medicamentos WHERE paciente_id = ?',
                [pacienteId]
            );
